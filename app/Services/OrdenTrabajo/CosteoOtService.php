    ...
    public function calcular(OrdenTrabajo $ot): array
    {
        ...
        foreach ($ot->tareas as $tarea) {
            foreach ($tarea->insumos as $linea) {
                ...
                $monto = (float) $linea->cantidad * (float) $costoUnit;
                $repuestos += $monto;

                if ($solicitud?->estado !== 'entregada') {
                    $repuestosEstimados += $monto;
                }
            }
        }
        ...
        return [
            ...
            'repuestos_estimados' => round($repuestosEstimados, 2),
